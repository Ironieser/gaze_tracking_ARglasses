...
class GazeTracking(object):
    ...
    def find_best_threshold(self, frame):
        ...
        best_threshold, iris_size = min(
            trials.items(), key=(lambda p: abs(p[1] - self.average_iris_size))
        )
    ...
    def find_iris_cnt(new_frame):
        ...
        _, contours, _ = cv2.findContours(
            new_frame, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE
        )
        # 将找出的轮廓按面积排序
    ...
    def detect_iris(self, iris_cnt):
        ...
        cv2.line(
            self.frame, (self.x - 3, self.y), (self.x + 3, self.y), color, thickness=2
        )
        cv2.line(
            self.frame, (self.x, self.y - 3), (self.x, self.y + 3), color, thickness=2
        )
