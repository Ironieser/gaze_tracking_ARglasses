...
class AreaRecognize(object):
    ...
    def recognize(self, image_dir, sess):
        ...
        predictions = sess.run(
            softmax_tensor, feed_dict={"DecodeJpeg/contents:0": self.image_data}
        )
        predictions = np.squeeze(predictions)

        top_k = predictions.argsort()[-self.top_k :]
        self.rec_num = len(top_k)
        self.rec_result = pd.DataFrame(
            np.zeros((self.rec_num, 2)), columns=["label", "score"]
        )
        count = 0
        for node_id in top_k:
            ...
            self.rec_result.iloc[count, 0], self.rec_result.iloc[count, 1] = (
                label,
                score,
            )
            count += 1
...
if __name__ == "__main__":
    ...
    area_recognize = AreaRecognize(top_k=5)
    with tf.Session() as sess:
